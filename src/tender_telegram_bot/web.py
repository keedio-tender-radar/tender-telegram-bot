...
@app.post("/send-results-digest")
async def send_results_digest(x_run_token: str | None = Header(default=None)) -> dict:
    """Digest periódico del pipeline (win-rate) a los chats. Omite si aún no hay decisiones."""
    if settings.run_token and x_run_token != settings.run_token:
        raise HTTPException(401, "Token de ejecución inválido o ausente.")
    if not settings.telegram_chat_id:
        raise HTTPException(400, "Falta TELEGRAM_CHAT_ID (destino del informe).")
    ...
    if not summary or summary.get("total_decisions", 0) == 0:
        _report_run("results-digest", "ok", count=0)
        return {"sent": 0, "reason": "sin decisiones registradas"}
    text = messages.format_outcomes(summary)
    chats = settings.telegram_chat_ids
    for chat in chats:
        await _application.bot.send_message(chat, text, disable_web_page_preview=True)
    _report_run("results-digest", "ok", count=summary.get("total_decisions"))
    return {"sent": len(chats), "decisions": summary.get("total_decisions")}
...
async def webhook(request: Request) -> dict:
    ...
    # Procesa el update EN LÍNEA (await) antes de responder a Telegram. Con scale-to-zero esto es
    # obligatorio: si solo encoláramos y respondiéramos al instante, la máquina puede dormirse antes
    # de que el consumidor de la cola ejecute el handler → los comandos se pierden en silencio.
    # Para no superar el timeout del webhook de Telegram, la máquina se mantiene caliente con un
    # cron keep-warm y los handlers lentos (/preguntar) avisan primero y usan timeouts acotados.
